...
IF EXISTS (select top 1 1 from [dbo].[MessageRecipient])
...
GO
PRINT N'The following operation was generated from a refactoring log file c5a0a597-f99a-46fa-999f-107634f35634';

PRINT N'Rename [dbo].[MessageRecipient].[UserId] to ArtistId';


GO
EXECUTE sp_rename @objname = N'[dbo].[MessageRecipient].[UserId]', @newname = N'ArtistId', @objtype = N'COLUMN';


GO
PRINT N'Dropping FK_MessageRecipient_Message...';
...
CREATE TABLE [dbo].[tmp_ms_xx_MessageRecipient] (
    ...
    PRIMARY KEY CLUSTERED ([MessageRecipientId] ASC)
);

IF EXISTS (SELECT TOP 1 1 
           FROM   [dbo].[MessageRecipient])
    BEGIN
        SET IDENTITY_INSERT [dbo].[tmp_ms_xx_MessageRecipient] ON;
        INSERT INTO [dbo].[tmp_ms_xx_MessageRecipient] ([MessageRecipientId], [MessageId], [ArtistId], [MessageRead], [MessageStatus], [Folder])
        SELECT   [MessageRecipientId],
                 ...
                 [Folder]
        FROM     [dbo].[MessageRecipient]
        ORDER BY [MessageRecipientId] ASC;
        SET IDENTITY_INSERT [dbo].[tmp_ms_xx_MessageRecipient] OFF;
    END
...
GO
PRINT N'Creating FK_MessageRecipient_Message...';


GO
ALTER TABLE [dbo].[MessageRecipient] WITH NOCHECK
    ADD CONSTRAINT [FK_MessageRecipient_Message] FOREIGN KEY ([MessageId]) REFERENCES [dbo].[Message] ([MessageId]);
